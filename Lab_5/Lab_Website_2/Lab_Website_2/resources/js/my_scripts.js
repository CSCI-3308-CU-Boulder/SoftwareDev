...
function loadStatsPage(){
	...
	for(i=2; i<table.rows.length; i++){
		...
		if(table.rows[i].cells[2].innerHTML > table.rows[i].cells[3].innerHTML && table.rows[i].cells[2].innerHTML!= 7){
			wins=wins+1;
			table.rows[i].cells[4].innerHTML = "Colorado Boulder"
...
function switchPlayers(playerNum){
	...
	var avg_p_yards = Math.floor(players[playerNum].pass_yards/players[playerNum].games_played);
	var avg_r_yards= Math.floor(players[playerNum].rushing_yards/players[playerNum].games_played);
	var avg_rec_yards= Math.floor(players[playerNum].receiving_yards/players[playerNum].games_played);
...
function loadPlayersPage(){
	var p_menu =  document.getElementById("player_selector");
	var add= "";

	for(var i=0; i < players.length; i++){
		add=add + "<a class= 'dropdown-item' href='#' onclick= 'switchPlayers("+ i +")'>"+players[i].name+ "</a>"
	}

	p_menu.innerHTML= add;
}
